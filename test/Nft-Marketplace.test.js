    ...
    : describe("Nft Marketplace Unit Tests", function () {
          ...
          describe("listItem", function () {
              it("emits an event after listing an item", async function () {
                  expect(await nftMarketplace.listItem(IpfsNft.address, TOKEN_ID, PRICE)).to.emit(
                      "ItemAdded"
                  )
              })
          ...
          describe("cancelListing", function () {
              ...
              it("emits event and removes listing", async function () {
                  await nftMarketplace.listItem(IpfsNft.address, TOKEN_ID, PRICE)
                  expect(await nftMarketplace.cancelListing(IpfsNft.address, TOKEN_ID)).to.emit(
                      "ItemCanceled"
                  )
                  const listing = await nftMarketplace.getListing(IpfsNft.address, TOKEN_ID)
          ...
          describe("buyItem", function () {
              ...
              it("transfers the nft to the buyer and updates internal proceeds record", async function () {
                  ...
                  nftMarketplace = nftMarketplaceContract.connect(user)
                  expect(
                      await nftMarketplace.buyItem(IpfsNft.address, TOKEN_ID, { value: PRICE })
                  ).to.emit("ItemBought")
          ...
          describe("updateListing", function () {
              ...
              it("updates the price of the item", async function () {
                  ...
                  await nftMarketplace.listItem(IpfsNft.address, TOKEN_ID, PRICE)
                  expect(
                      await nftMarketplace.updateListing(IpfsNft.address, TOKEN_ID, updatedPrice)
                  ).to.emit("ItemListed")
                  const listing = await nftMarketplace.getListing(IpfsNft.address, TOKEN_ID)
