...
!developmentChains.includes(network.name)
  ...
  : describe("Nft Marketplace Unit Tests", function () {
      ...
      it(" aklfnhnadohfon", async () => {
        const nftTx = await IpfsNft.requestNft();
        const nftReceipt = await nftTx.wait(1);
        console.log((await IpfsNft.getLastRange()).toString());
        const nftTx2 = await IpfsNft.requestNft();
        const nftReceipt2 = await nftTx2.wait(1);
        console.log((await IpfsNft.getLastRange()).toString());
        const nftTx3 = await IpfsNft.requestNft();
        const nftReceipt3 = await nftTx3.wait(1);
        console.log((await IpfsNft.getLastRange()).toString());
      });
      ...
      //     const something = await IpfsNft.tokenURI("0");
      //     //const alsoSomething = await IpfsNft.tokenURI("1");
      //     console.log("Something ", something);
      //     assert(true);
      //     //console.log("AlsoSomething", alsoSomething);
      //     // expect(
      //     //   await nftMarketplace.listItem(IpfsNft.address, "1", "10")
      //     // ).to.emit("ItemAdded");
      //   });
