...
contract GameContract{
    ...
    event GameStarted(
        address indexed signer,
        uint256 indexed remainingToken    
    );
    event TokenBought(
        address indexed signer, 
    ...
    constructor(uint256 initialTokenSupply, uint256 tokenNeededToPlay, address priceFeed){
        owner = msg.sender;
    ...
    function burn(address signer) public returns (uint256 isApproved){
        ...
        s_addressToToken[signer].tokenAmount = s_addressToToken[signer].tokenAmount-tokenNeededToPlay;
    ...
    function buyToken(address signer) public payable {
        ...
        (bool callSuccess, ) = payable(this).call{
            value: msg.value
        }();
        if(!callSuccess){
        ...
        s_addressToToken[signer].tokenAmount = s_addressToToken[signer].tokenAmount + tokenToTransfer;
